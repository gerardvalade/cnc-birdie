...
{ // -----------------------------------------------
	...
	union() {
		translate([0, 0, -l/2-h+0.02]) cylinder(r=(orad+cld/2), h=l,  center=true);
		if (h>0)
...
{ // -----------------------------------------------
	...
	bom(str("screw-",name), str("Screw ",name), ["hardware"]);
	...
	difference() {
		...
		translate([0,0,head_height+0.02]) key_slot(k=key_width, l=key_depth);
	}
...
{ // -----------------------------------------------
	...
	bom(str("nut-",name), str("Nut ",name), ["hardware"]);
